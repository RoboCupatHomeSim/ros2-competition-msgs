...
private:
  ...
  void messageCallback(const interactive_cleanup_msgs::msg::InteractiveCleanupMsg::ConstSharedPtr message)
  {
  ...
  void sendMessage(rclcpp::Publisher<interactive_cleanup_msgs::msg::InteractiveCleanupMsg>::SharedPtr &publisher, const std::string &message)
  {
...
public:
  ...
  {
    node_ = std::make_shared<rclcpp::Node>("interactive_cleanup_sample");
    ...
    RCLCPP_INFO(node_->get_logger(), "Interactive Cleanup Sample start!");

    auto sub_msg = node_->create_subscription<interactive_cleanup_msgs::msg::InteractiveCleanupMsg>("/interactive_cleanup/message/to_robot", 100, std::bind(&InteractiveCleanupSample::messageCallback, this, std::placeholders::_1));
    auto pub_msg = node_->create_publisher   <interactive_cleanup_msgs::msg::InteractiveCleanupMsg>("/interactive_cleanup/message/to_moderator", 10);
...
int main(int argc, char **argv)
{
  ...
  int ret = interactive_cleanup_sample.run(argc, argv);
